...
public:
  ...
  vector<coordinate> getCurrentImage(/*unsigned char * src, Matrix& rMat, Matrix& gMat, Matrix& bMat, int mode = 255*/) const;
...
{ 
  ...
  Matrix gMat;
  Matrix bMat;
 // getCurrentImage(src, rMat, gMat, bMat, mode);
 // writePPM(filename, rMat, gMat, bMat);  
}

vector<coordinate> MultiObjectTLD::getCurrentImage(/*unsigned char * src, Matrix& rMat, Matrix& gMat, Matrix& bMat, int mode*/) const
{ 
  /*int size = ivHeight * ivWidth;
  ...
  for (int i = 0; i < ivNObjects; i++)
  {
	  ...
	  {
		  {
			  m_coordinate.x = ivCurrentBoxes[i].x;
			  m_coordinate.y = ivCurrentBoxes[i].y;
			  m_coordinate.width = ivCurrentBoxes[i].width;
			  m_coordinate.height = ivCurrentBoxes[i].height;
		  }
		  /* rMat.drawBox(ivCurrentBoxes[i], 255);
		   gMat.drawBox(ivCurrentBoxes[i], (ivValid[i] ? 0 : 255));
		   bMat.drawBox(ivCurrentBoxes[i], 0);*/
		  m_vecCoordiante.push_back(m_coordinate);
		  /* if (ivNObjects > 1)
		   {*/
		  ...
		  //cout << m_objectBoxes[m_objectBoxes.size()-1].objectId << endl;
		  /*}*/
	  }
  }
